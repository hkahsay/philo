...
/*   Updated: 2023/03/02 17:14:47 by hkahsay          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_info	*set_philos(t_info	*info)
{
	...
	while (i < info->nbr_of_philosophers)
	{
		...
		pthread_mutex_init(&info->philo[i].mutex_meal_count, NULL);
		pthread_mutex_init(&info->philo[i].mutex_eat, NULL);
		pthread_mutex_init(&info->philo[i].r_fork, NULL);
		pthread_mutex_init(&info->mutex_dead_stop, NULL);
		if (i != 0)
...
int	create_threads(t_info *info)
{
	...
	i = 0;
	while (i < info->nbr_of_philosophers && !info->stop)
	{
	...
	if (pthread_create(&info->philo->tid_dead, NULL, &check_dead, \
		(void *) info->philo))
		{
			return (-1);		
	...
	pthread_join(info->philo->tid_dead, NULL);
	return (0);
}
